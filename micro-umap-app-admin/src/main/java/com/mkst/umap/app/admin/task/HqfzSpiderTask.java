 ...
 */
@Slf4j
@Component("hqfzSpiderTask")
public class HqfzSpiderTask {

	@Autowired
	private ArticleMapper articleMapper;

	public void spiderSpp(String params) {
		log.info("爬取最高检环球法制定时任务开始");
		try {
			List<String> startUrls = new ArrayList<String>();
			startUrls.add("https://www.spp.gov.cn/spp/dfjcdt/index.shtml");
//			for (int i = 2; i<= 34; i++) {
//				startUrls.add("https://www.spp.gov.cn/spp/dfjcdt/index_"+i+".shtml");
//			}
			
			HunterConfig config = new HunterConfig();
			config.setEntryUrls(startUrls)
					.setTargetLinksRegex("https://www\\.spp\\.gov\\.cn/spp/dfjcdt/[0-9]{6}/[\\w\\d]+\\.shtml")
			...
			if (null == list || list.isEmpty()) {
				log.warn("爬取最高检环球法制定时任务结束：获取数据为空");
			} else {
				Article article = null;
				for (VirtualArticle virtualArticle : list) {
					// 检查文章是否已经存在
					article = new Article();
					article.setCategoryId("71"); // 环球法制分类：71
			...
			log.info("爬取最高检环球法制定时任务结束：同步数据{}条", n);
		} catch (Exception e) {
			log.error("爬取最高检环球法制定时任务结束：获取数据失败：{}", e.getMessage(), e);
		}
	}

	public static void main(String[] args) {
		new HqfzSpiderTask().spiderSpp("3");
	}
}
