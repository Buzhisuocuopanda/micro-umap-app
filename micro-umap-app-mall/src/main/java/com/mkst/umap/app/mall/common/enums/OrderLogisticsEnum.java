package com.mkst.umap.app.mall.common.enums;
...
public enum OrderLogisticsEnum implements IEnum<String> {
	 ...
	 * @return
	 */
	public static List<SysDictData> queryAll(String prefix) {
		List<SysDictData> list = new ArrayList<>();
		for (OrderLogisticsEnum t : OrderLogisticsEnum.values()) {
			if(t.name().contains(prefix)){
				SysDictData sysDictValue = new SysDictData();
				sysDictValue.setDictValue(t.getValue());
				sysDictValue.setDictLabel(t.getDesc());
